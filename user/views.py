...
from user.models import User, UserPhoto, UserRole, CoinSettings, PrivateUserPhoto
...
class TokenLoginView(LoginView):
    ...
    def get_response(self):
        ...
        user_details_serializer.update(instance=self.user,validated_data={'isOnline': True,"user_last_seen": timezone.now()}  )       
        data = {
...
class WorkerListView(
    ...
    queryset = (
        User.objects.all()
        .filter(Q(roles__role__in=["CHATTER"]))
...
class ModeratorListView(
    ...
    queryset = (
        User.objects.all()
        .filter(Q(roles__role__in=["MODERATOR"]))
        .distinct("id")
    )
    serializer_class = serializers.UserSerializer
...
class AdminListView(
    ...
    queryset = (
        User.objects.all()
        .filter(Q(roles__role__in=["ADMIN"]))
        .distinct("id")
    )
    serializer_class = serializers.UserSerializer
...
class WorkerSignupView(APIView):
    def post(self, request):
        ...
        try:
            ...
            print(e)
            return Response({"error": "username already exists"}, status=500)
