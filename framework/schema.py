...
class CreateUser(graphene.Mutation):
    ...
    def mutate(self, info, username, password, email, onesignal_player_id=None, fcm_registration_id=None):
        user = get_user_model()(
            username=username,
            email=email,
...
class UpdateProfile(graphene.Mutation):
    ...
    def mutate(self, info, id, username=None, fullName=None, language=None,user_language_code=None, gender=None, email=None, height=None, familyPlans=None,
               about=None, location=None, age=None, avatar_index=None, isOnline=None, tag_ids=None, url=None, platform=None,
               politics=None, zodiacSign=None, interested_in=None, ethinicity=None, religion=None, education=None,
               photos=None, onesignal_player_id=None, fcm_registration_id=None, music=None, likes=None, book=None, movies=None, sportsTeams=None,
        ...
        if language is not None:
            languageObj = Language.objects.filter(id=language).first()
            if languageObj:
                user.language.set([languageObj.id])
        ...
        if location is not None:
            ...
            country,country_code = get_country_from_geo_point(geo_point=location)
    ...
    def mutate(self, info, id):
        try:
            ...
            if info.context.user.id!=u.id:
                ...
                Reported_Users.objects.filter(Q(reporter=u) | Q(reportee=u)).delete()
                u.delete()
...
class Query(
    ...
    def resolve_users(self, info, name=None):
        if name:
            ...
            return get_user_model().objects.filter(Q(social_auth__isnull=False) | Q(social_auth__isnull=True)).filter(fullName__icontains=name).exclude(id__in=blocked).exclude(id__in=blocked_by)
        return get_user_model().objects.filter(Q(social_auth__isnull=False) | Q(social_auth__isnull=True)).exclude(id__in=blocked)
    ...
    def resolve_user(self, info, **kwargs):
        id = kwargs.get('id')
        if id is None:
            raise Exception('id i`s a required parameter')
        try:
            print(" resolve_user like 599: framework.schema")
            return get_user_model().objects.get(id=id)
        except User.DoesNotExist as e:
            raise Exception("User doesn't exists.") from e
    ...
    def resolve_random_users(self, info, **kwargs):
        ...
        if interest is not None:
            ...
            filtered_users = [user.id for user in users if interest in user.interestedIn_display]

            filtered_users = list(set(filtered_users))
            blocked = info.context.user.blockedUsers.all().values_list('id', flat=True)
            blocked_by = info.context.user.blocked_by.all().values_list('id', flat=True)
            res = get_user_model().objects.filter(id__in=filtered_users).exclude(id__in=blocked)
        else:
            blocked = info.context.user.blockedUsers.all().values_list('id', flat=True)
            blocked_by = info.context.user.blocked_by.all().values_list('id', flat=True)
        ...
        if max_distance and latitude and longitude:
            ...
            for user in res:
                ...
                if geopy.distance.distance(base_location, user_location).miles <= max_distance:
                    user.distance = str(int(geopy.distance.distance(base_location, user_location).miles))
                    user.save()
...
class Subscription(chat.schema.Subscription,moments.schema.Subscription, graphene.ObjectType):
    pass
