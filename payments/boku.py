...
from payments.models import PrivateKey, PublicKey
...
class Boku(object):
    ...
    def decode_payment_request(self, request):
        ...
        public_key = public_key.objects.filter(payment_gateway="boku").last().key_text.encode()
    ...
    def make_get_request(self, url):
        ...
        private_key = PrivateKey.objects.filter(payment_gateway="boku").last().key_text.encode()
    ...
    def make_post_request(self, url, body):
        ...
        # GENERATE TOKEN
        token = self.generate_payment_token(private_key=private_key, body_sha256=body_sha256)
        ...
        response = requests.post(url, headers=headers, data=json.dumps(body, sort_keys=True)).json()
    ...
    def make_put_request(self, url, body):
        ...
        response = requests.put(url, headers=headers, data=json.dumps(body, sort_keys=True)).json()
