...
class reportUser(graphene.Mutation):
    ...
    def mutate(self, info, reporter, reportee, reason):
        ...
        if reporter_user == reportee_user:
            raise Exception("You can not report yourself")

        report = Reported_Users.objects.create(
            reporter=reporter_user,
            reportee=reportee_user,
            reason=reason
        )
...
# ==========================================
            if "apple" in provider.lower():
                if access_verifier == '':
                    access_verifier_check = UserSocialAuth.objects.filter(extra_data=access_token)
                    if access_verifier_check.count()>0:
                        access_verifier_check = UserSocialAuth.objects.get(extra_data=access_token)
                        access_verifier = access_verifier_check.uid
                    else:
                        return Exception("Email is required for first sign in.")
