...
from .models import age, ethnicity, politics, religious, Language, family, zodiacSign, tags, interestedIn, gender, height, \
    searchGender, config
...
class AllPickers(graphene.ObjectType):
    ...
    def resolve_religiousPicker(self, info):
        return religious.objects.values(
            'id', value=F(translated_field_name(info.context.user, 'religious')),
    ...
    def resolve_tagsPicker(self, info):
        return tags.objects.values(
            'id', value=F(translated_field_name(info.context.user, 'tag')),
            value_fr=F('tag_fr'),
    ...
    def resolve_zodiacSignPicker(self, info):
        return zodiacSign.objects.values(
            'id', value=F(translated_field_name(info.context.user, 'zodiacSign')), 
